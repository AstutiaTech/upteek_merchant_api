...
def get_media(db: Session, filters: Dict={}):
    query = db.query(Medium)
    if 'merchant_id' in filters:
        query = query.filter_by(merchant_id = filters['merchant_id'])
    if 'mediumable_type' in filters:
        query = query.filter_by(mediumable_type = filters['mediumable_type'])
    if 'mediumable_id' in filters:
        query = query.filter_by(mediumable_id = filters['mediumable_id'])
    if 'status' in filters:
        query = query.filter_by(status = filters['status'])
    if 'created_by' in filters:
        query = query.filter_by(created_by = filters['created_by'])
    return query.filter(Medium.deleted_at == None).order_by(desc(Medium.id))
